...
In this stack, a \code{push} and a \code{pop} can exchange their value without using the stack. A \code{pop} executed by a thread $t$ choose a cell of the array \code{E}. If the value of the flag in this cell is equal to \code{F} (Final), then the push can change it to \code{W} meaning that he is waiting for exchanging its value with a \code{push}. A \code{push} executed by a second thread $t'$ can 
